...
import { updateOrderStatusAdmin, refundOrder } from "@/app/actions/admin";
...
export function OrderActions({ orderId, currentStatus }: OrderActionsProps) {
  ...
  function handleAction(status: "paid" | "cancelled") {
  ...
  function handleRefund() {
    if (!confirm("Tem certeza que deseja reembolsar este pedido? Esta ação será processada no gateway de pagamento.")) return;
    setRefundError(null);
    startTransition(async () => {
      const result = await refundOrder(orderId);
      if (!result.success) {
        setRefundError(result.error ?? "Erro ao processar reembolso.");
      }
    });
  }
