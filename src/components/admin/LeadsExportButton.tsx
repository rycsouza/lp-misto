...
import { exportLeadsCSV } from "@/app/actions/admin-growth";
...
export function LeadsExportButton({ source }: LeadsExportButtonProps) {
  const [loading, setLoading] = useState(false);

  async function handleExport() {
    setLoading(true);
    try {
      const csv = await exportLeadsCSV(source);
      const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `leads${source ? `-${source}` : ""}.csv`;
