...
/** Relatório pós-jogo — comparecimento de um jogo. Aba "Pós-jogo" da tela de relatórios. */
export async function PostGameReport({ game: gameParam }: { game?: string }) {
  ...
  const kpis = report
    ? [
        { label: "Público presente", value: String(report.validated), icon: Users, accent: "text-green-500" },
        ...
        { label: "Receita (ingressos)", value: brl(report.revenueCents), icon: TrendingUp, accent: "text-green-500" },
      ]
    : [];
        ...
        <>
          ...
          <div className="bg-card border border-border rounded-xl px-5 py-4">
            <p className="text-lg font-bold text-foreground">
              {report.game.competition} · {siteName} <span className="text-muted-foreground">×</span>{" "}
              {report.game.opponent}
            </p>
          ...
          {!report.hasData ? (
            <div className="bg-card border border-border rounded-xl p-8 text-center">
              <p className="text-foreground font-medium">
                Sem ingressos/validações registrados para este jogo.
              </p>
                ...
                <div className="bg-card border border-border rounded-xl overflow-hidden">
                  ...
                  <ul className="sm:hidden divide-y divide-border/50">
                    {report.byType.map((t) => {
                      ...
                      return (
                        <li key={t.typeCode + t.typeName} className={`px-4 py-3 ${t.vip ? "bg-orange-500/5" : ""}`}>
                          <div className="flex items-center justify-between gap-2 mb-1.5">
                            <span className="inline-flex items-center gap-1.5 text-foreground text-sm font-medium min-w-0">
                              {t.vip && <Star size={12} className="text-orange-500 shrink-0" />}
                              <span className="break-words">{t.typeName}</span>
                            </span>
                            <span className="shrink-0 text-sm font-semibold text-foreground tabular-nums">{tpct}%</span>
                          </div>
                          <div className="flex gap-3 text-xs text-muted-foreground tabular-nums">
                            <span>Presentes: <b className="text-foreground">{t.validated}</b></span>
                            <span>Emit.: {t.emitted}</span>
                            <span>No-show: {t.noShow}</span>
                          </div>
                        </li>
                      );
                    })}
                ...
                <div className="bg-card border border-border rounded-xl p-5">
                  ...
                  <div className="flex items-end gap-2 h-40 overflow-x-auto">
                    {report.timeline.map((t) => (
                      <div key={t.hour} className="flex-1 flex flex-col items-center gap-1.5 min-w-[24px]">
